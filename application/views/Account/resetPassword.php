<?php
$this->load->view('layout/header');
?> 
<section class="image_bg_8 darkness type_4 relative">
    <div class="container">
        <div class="">
            <div class="col-lg-6 col-md-6 col-sm-6 f_none d_table_cell v_align_m d_xs_block m_xs_bottom_30 form_description">

            </div>
            <div class="pull-right col-lg-6 col-md-6 col-sm-6 f_none d_table_cell v_align_m d_xs_block t_align_c">
                <div class="create_account_form_wrap r_corners d_inline_b w_xs_full">
                    <h4 class="fw_light color_dark m_bottom_23">Reset Password</h4>
                    <p>Enter your email address to reset your login password.</p>
                    <form class="create_account_form" method="post" action="#">
                        <ul>
                            <li class="m_bottom_10 m_xs_bottom_15 relative">
                                <i class="icon-mail-alt login_icon fs_medium color_grey_light_2"></i>
                                <input type="email" name="email" placeholder="Email as Username" class="r_corners bg_light w_full border_none">
                            </li>
                            <li class="t_align_c">
                                <button name="forget" class="button_type_3 d_inline_b color_purple r_corners tr_all fw_light">Submit</button>
                            </li>
                        </ul>
                    </form>
                </div>
            </div>
        </div>
    </div>
</section>
<script language='JavaScript' type='text/javascript'>
    function refreshCaptcha()
    {
        var img = document.images['captchaimg'];
        img.src = img.src;
    }

<?php
if ($checkmail == "yes") {
    ?>
        $(document).ready(function () {
            swal({
                type:"success",
                title: "Request Mail Sent!",
                text: "Check Email In Your Inbox",
                timer: 2500,
                onClose: () => {
                    window.location = '<?php echo site_url("/"); ?>';

                }
            })
        })
    <?php
}
?>
</script>
<?php
$this->load->view('layout/footer');
?>
