                        ...
                        <table class="table table-striped table-bordered table-hover filterTable" >
                            <thead>
                                <tr style="font-size: 12px">
                                    <th><b>S. No.</b></th>
                                    <th><b>Order No.<br/>Invoice No</b></th>
                            ...
                            <tbody>
                                ...
                                for ($i = 0; $i < count($invoicedata); $i++) {
                                    ...
                                    <tr style="font-size: 12px">
                                        ...
                                        <td><?php echo $orderData['order_no']; ?></br><?php echo str_replace("ON", "IN", $orderData['order_no']); ?></td>

                                        <td>
                                            <?php
                                            foreach ($orderData['products'] as $key => $value) {
                                                echo "<li>" . $value['item_code'] . ' - ' . $value['tag_title'] . " - {{'" . $value['total_price'] . "'|currency}}</li>";
                                            }
                                            ?>
                                        </td>
                                        ...
                                        <td>
                                            {{'<?php echo $orderData['subtotal']; ?>'|currency}}
                                        </td>
                                        <td>{{'<?php echo $orderData['shipping_amount']; ?>'|currency}}
                                        </td>
                                        <td>
                                            <?php echo $orderData['total_price'] ?>
                                        </td>

                                        <td> 
                                            <a href="<?php echo site_url("Order/orderdetails/".$orderData['id']); ?>" class="btn btn-default btn-xs"> View</a>
                                        </td>  
